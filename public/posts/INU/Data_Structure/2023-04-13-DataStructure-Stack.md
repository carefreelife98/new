...
스택 (Stack) 이란?
```
<img src="/assets/images/INU/danbistack.jpeg" alt="danbistack_Procdess" /><br>`사진출처: 우리집`<br><br>
...
- **`"스택이란, 쌓여있는 객체이다."`**
  ...
  - 이처럼 스택이란 **`어떠한 객체들이 쌓여있는 형태`** 를 말한다.
...
스택의 특징
```
<img src="/assets/images/INU/stacklifo.png" alt="stacklifo_Procdess" />`후입선출(LIFO: Last-In First-Out)`<br>
- 위 사진처럼 스택에서의 입출력은 가장 윗단에서만 일어나고 그 외의 위치에서는 데이터의 삭제가 이루어지지 못한다.
- **후입선출**
  - 자료구조에서의 스택은 자료의 출력 순서가 입력 순서의 역순으로 이루어져야 할 경우에 매우 요긴하게 사용된다.
  - ex. (a, b, c, d, e) 형태의 데이터를 (e, d, c, b, a) 와 같은 역순으로 정렬하고 싶을때
...
스택의 구조
```
<img src="/assets/images/INU/stackstructure.png" alt="stackstructure_Procdess" />`스택의 구조`
...
- 함수는 들어온 길을 되돌아가야 하기 때문이다.
  - 즉, **`호출된 순서의 역순으로 빠져나가야 한다.`**
- 이처럼 스택은 함수가 복귀할 주소를 저장하는 데에 사용된다.


**시스템 스택**
<img src="/assets/images/INU/systemstack.png" alt="systemstack_Procdess" />`함수 호출 과정에서의 스택 응용(시스템 스택)`
...
3. 자기 자신을 호출하는 재귀 함수 (recursion) 도 동일한 방법으로 활성 레코드가 생성된 후 사라진다.
...
<img src="/assets/images/INU/calstack.png" alt="calstack_Procdess" />`스택의 push() & pop() 연산 과정`<br><br>
...
배열을 이용해 스택을 구현해보자.
```
<img src="/assets/images/INU/arrstack.png" alt="arrstack_Procdess" />`배열로 구현한 스택의 구조`<br><br>
- 배열에는 다양한 type의 요소들을 넣을 수 있다. (예제: int형 정수)
  - int형 정수를 저장해야 하니 int형의 1차원 배열 stack[MAX_STACK_SIZE]을 생성.
- 스택에서 가장 최근에 입력된 데이터를 가리키는 변수 top 도 생성.
  - 가장 먼저 입력된 순서대로 stack[0] 부터 stack[top] 까지 저장 될 것이다.
  - top 변수는 스택이 비어있으면 -1의 값을 가진다.
    - 스택이 공백 상태일때 top이 0의 값을 가진다면, stack[0]에 값이 있다는 것으로 해석이 되기 때문.

```
top 변수의 특성에 유의해 스택의 공백, 포화 상태 검사 함수를 의사코드로 알아보자.
```
<img src="/assets/images/INU/arrstackempfull.png" alt="arrstackempfull_Procdess" />`is_empty , is_full 연산 함수의 구현`<br><br>
...
<img src="/assets/images/INU/stackpushpop.png" alt="stackpushpop_Procdess" />`push() 연산 함수의 구현`<br><br>
...
<img src="/assets/images/INU/arrstackpop.png" alt="arrstackpop_Procdess" />`pop() 연산 함수의 구현`<br><br>
...
<img src="/assets/images/INU/arrstackrs.png" alt="arrstackrs_Procdess" />`동적 배열 스택 프로그램 실행 결과`
<br><br>
...
### 괄호 검사 프로그램: 알고리즘
<img src="/assets/images/INU/stackmatching.png" alt="stackmatching_Procdess">`괄호 검사 프로그램의 알고리즘`
<br><br>
- 알고리즘의 개요
...
<img src="/assets/images/INU/matchingstack.png" alt="matchingstack_Procdess">`괄호 검사 프로그램 실행 결과`
...
<img src="/assets/images/INU/postfix.png" alt="postfix_Procdess">`수식의 표기 방법`<br><br>
- **컴퓨터에서의 수식 계산 순서**
...
<img src="/assets/images/INU/calpostfix.png" alt="calpostfix_Procdess">`후위 표기식의 계산 (82/3-32*+)`<br><br>
1. 수식을 왼쪽에서 오른쪽으로 스캔 해나간다.
2. 피연산자 이면 스택에 저장.
3. 연산자를 만나는 순간 필요한 만큼의 피연산자를 스택에서 꺼내 연산을 실행하고, 그 결과를 다시 스택에 저장한다.<br>

<img src="/assets/images/INU/calpostfix2.png" alt="calpostfix2_Procdess">`후위 표기식의 계산 과정 (82/3-32*+)`<br><br>
...
<img src="/assets/images/INU/rspostfix.png" alt="rspostfix_Procdess" />`후위 표기식 프로그램의 실행 결과 (82/3-32*+)`<br><br>
...
<img src="/assets/images/INU/inpostfix.png" alt="inpostfix_Procdess" />`후위 표기식 프로그램의 실행 결과 (82/3-32*+)`<br><br>
...
<img src="/assets/images/INU/in_postfix.png" alt="in_postfix_Procdess" />`후위 표기식 변환 프로그램의 실행 결과 (2+3)*4+9`<br><br>
...
<img src="/assets/images/INU/Traquair_House_Maze.jpg" alt="Traquair_House_Maze_Procdess" />`사진출처`[Maze](https://ko.wikipedia.org/wiki/%ED%8C%8C%EC%9D%BC:Traquair_House_Maze.jpg)<br><br>
- 미로의 출구를 찾는 방법 중 가장 기본적인 방법은 시행착오 방법이다.
  - **`시행착오 방법: 하나의 경로를 선택하여 한번 시도해보고 안되면 다시 다른 경로를 시도.`**
...
<img src="/assets/images/INU/rs_maze.png" alt="rs_maze_Procdess" /><br>`미로 탐색 프로그램의 실행 결과`
